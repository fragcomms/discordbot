...
/**
 * PYTHON BRIDGE
 * Executes the Python script to handle SCP and Database operations.
 */
async function runPythonWorker(wavPath: string, remoteDir: string, remoteFileName: string, payload: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const pythonProcess = spawn('python3', [
      path.join(__dirname, 'cleanup', 'cleanup.py'),
      wavPath,
      remoteDir,
      remoteFileName,
      payload
...
export async function cleanUpProcess(guildId: string, channelId: string, voiceChannelId: string, client: DiscordClient) {
  ...
  // Create a simplified JSON object to pass to Python
  // We cannot pass complex Discord objects, only raw data
  const payload = JSON.stringify({
    timestamp: timestamp,
    users: guildRecordings.map(r => ({
      id: r.user.id,
  ...
  try {
    await runPythonWorker(wavPath, remoteDir, remoteFileName, payload);
    console.log("Python worker completed successfully.");
  } catch (err) {
    console.error("Python worker failed:", err);
    sendMessage(client, channelId, "Audio processed, but storage/database operations failed.");
  }
